...
var hdInsightCommandLine = function (cli, userInteractor, executionProcessor) {
  ...
  if (executionProcessor) {
    ...
    this.createClusterCommand = function(resourceGroupName, clusterName, location, osType, options, _) {
		...
		if(!utils.stringIsNullOrEmpty(options.hadoopConfigurationPath))
		{
			options.configurations = self.user.readConfig(options.hadoopConfigurationPath);
		}
		
		if(!utils.stringIsNullOrEmpty(options.scriptActionsPath))
		{
			options.scripActions = self.user.readConfig(options.scriptActionsPath);
		}
...
hdInsightCommandLine.init = function(cli) {
    ...
    cluster.command('create [resourceGroupName] [clusterName] [location] [osType]')
		...
		.option('--hadoopConfigurationPath <hadoopConfigurationPath>', $('Hadoop configuration value customization or a Hive shared library customization file path'))
		.option('--scriptActionsPath <scriptActionsPath>', $('Scrip actions file path'))
