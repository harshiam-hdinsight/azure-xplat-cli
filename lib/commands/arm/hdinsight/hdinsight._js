...
var hdInsightCommandLine = function (cli, userInteractor, executionProcessor) {
  ...
  if (executionProcessor) {
    ...
    this.createClusterCommand = function(clusterName, options, _) {
		...
		if(!utils.stringIsNullOrEmpty(options.hiveMetastoreSqlServerName)
		|| !utils.stringIsNullOrEmpty(options.hiveMetastoreDatabaseName)
		|| !utils.stringIsNullOrEmpty(options.hiveMetastoreUserName)
		|| !utils.stringIsNullOrEmpty(options.hiveMetastorePassword))
		{
			options.hiveMetastoreSqlServerName = self.user.promptIfNotGiven($('Hive metastore SQL Server name: '), options.hiveMetastoreSqlServerName, _);
			...
			options.hiveMetastore = {
				server: options.hiveMetastoreSqlServerName,
		...
		if(!utils.stringIsNullOrEmpty(options.oozieMetastoreSqlServerName)
		|| !utils.stringIsNullOrEmpty(options.oozieMetastoreDatabaseName)
		|| !utils.stringIsNullOrEmpty(options.oozieMetastoreUserName)
		|| !utils.stringIsNullOrEmpty(options.oozieMetastorePassword))
		{
			options.oozieMetastoreSqlServerName = self.user.promptIfNotGiven($('Oozie metastore SQL Server name: '), options.oozieMetastoreSqlServerName, _);
			...
			options.oozieMetastore = {
				server: options.oozieMetastoreSqlServerName,
    ...
    this.showClusterCommand = function(clusterName, options, _) {
        ...
        var result = self.processor.getCluster(options.resourceGroupName, clusterName, options, _);
    ...
    this.listClustersCommand = function(options, _) {
        ...
        var result = self.processor.listClusters(options.resourceGroupName, options, _);
        self.user.endProgress();
    ...
    this.deleteClusterCommand = function(clusterName, options, _) {
        ...
        self.processor.deleteCluster(options.resourceGroupName, clusterName, options, _);
    ...
    this.resizeClusterCommand = function (clusterName, targetInstanceCount, options, _) {
    	...
    	var result = self.processor.resizeCluster(options.resourceGroupName, clusterName, targetInstanceCount, options, _);
    ...
    this.enableHttpAccessCommand = function (clusterName, userName, password, options, _) {
        ...
        var result = self.processor.enableHttp(options.resourceGroupName, clusterName, userName, password, options, _);
    ...
    this.disableHttpAccessCommand = function (clusterName, options, _) {
    	...
    	var result = self.processor.disableHttp(options.resourceGroupName, clusterName, options, _);
	...
	this.enableRdpAccessCommand = function (clusterName, rdpUserName, rdpPassword, rdpExpiryDate, options, _) {
        ...
        var result = self.processor.enableRdp(options.resourceGroupName, clusterName, rdpUserName, rdpPassword, rdpExpiryDate, options, _);
    ...
    this.disableRdpAccessCommand = function (clusterName, options, _) {
    	...
    	var result = self.processor.disableRdp(options.resourceGroupName, clusterName, options, _);
...
hdInsightCommandLine.init = function(cli) {
    ...
    cluster.command('create [clusterName]')
        ...
        .option('--rdpAccessExpiry <rdpAccessExpiry>', $('RDP access expiry (only for Windows clusters)'))
        .option('--virtualNetworkId <virtualNetworkId>', $('(optional) Virtual network ID for the cluster (only for Linux clusters) '))
        .option('--subnetName <subnetName>', $('(optional) Subnet for the cluster (only for Linux clusters)'))
		.option('--additionalStorageAccounts <additionalStorageAccounts>', $('Optional additional storage accounts. Can be multiple. ' +
       	...
       	.option('--hiveMetastoreSqlServerName <hiveMetastoreSqlServerName>', $('(optional) SQL Server name for the external metastore for Hive'))
		...
		.option('--oozieMetastoreSqlServerName <oozieMetastoreSqlServerName>', $('(optional) SQL Server name for the external metastore for Oozie'))
	...
	config.command('add-hadoop-config [configFilePath]')
