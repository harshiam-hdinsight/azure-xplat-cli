...
__.extend(HdiClient.prototype, {
	...
	deleteCluster: function (resourceGroupName, clusterName, _) {
        try {
            var subscription = profile.current.getSubscription(this.subscription);
            var client = utils.createHDInsightManagementClient(subscription);
            return client.clusters.deleteCluster(resourceGroupName, clusterName, _);
        } catch (e) {
            if (e.code === 'ResourceNotFound' || e.code === 'NotFound') {
                return null;
            }
	...
	createConfigFile: function (configFilePath, options, _) {
        ...
        var result = configClient.createConfigFile(configFilePath, options, _);
        return result;
    },
  
  addConfigValue: function (configFilePath, options, _) {
        var configClient = new HdiConfigClient(this.cli);
        var result = configClient.addConfigValue(configFilePath, options, _);
        return result;
    },
  
  addScriptAction: function (configFilePath, options, _) {
        var configClient = new HdiConfigClient(this.cli);
        var result = configClient.addScriptAction(configFilePath, options, _);
        return result;
    },
});
