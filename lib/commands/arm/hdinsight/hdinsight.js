...
var ExecutionProcessor = function (cli) {
  ...
  this.createCluster = function (resourceGroupName, clusterName, createParams, options, _) {
    var hdInsight = new HdiClient(cli, options.subscription);
    var result = hdInsight.createCluster(resourceGroupName, clusterName, createParams, options.subscription, _);
    return result;
  };

  this.getCluster = function (resourceGroupName, clusterName, _) {
  ...
  this.deleteCluster = function (resourceGroupName, clusterName, _) {
    var hdInsight = self.createHdiClient(cli, options.subscription);
...
var hdInsightCommandLine = function (cli, userInteractor, executionProcessor) {
  ...
  this.createClusterCommand = function (resourceGroupName, clusterName, location, osType, options, _) {
      ...
      //options.resourceGroupName = cli.interaction.promptIfNotGiven($('Resource Group name: '), resourceGroupName, _);
      //options.clusterName = cli.interaction.promptIfNotGiven($('Cluster name: '), clusterName, _);
      //options.osType = cli.interaction.promptIfNotGiven($('OS type: '), osType, _);
      //options.defaultStorageAccountName = cli.interaction.promptIfNotGiven($('storage account url: '), options.defaultStorageAccountName, _);
      //options.defaultStorageAccountKey = cli.interaction.promptIfNotGiven($('storage account key: '), options.defaultStorageAccountKey, _);
      //options.defaultStorageContainer = cli.interaction.promptIfNotGiven($('storage container name: '), options.defaultStorageContainer, _);
      //options.dataNodeCount = cli.interaction.promptIfNotGiven($('Number of data nodes: '), options.dataNodeCount, _);
      //options.location = cli.interaction.promptIfNotGiven($('Data center location: '), location, _);
      //options.userName = cli.interaction.promptIfNotGiven($('User name: '), options.userName, _);
      //options.password = cli.interaction.promptIfNotGiven($('Password: '), options.password, _);
        ...
        var hdiVersion = 'default';
        //if (utils.stringIsNullOrEmpty(version)) {
        //    version = 'default';
        //}

        //if (utils.ignoreCaseEquals(osType, 'windows')) {
        //    headNodeSize = cli.interaction.promptIfNotGiven($('Head node size (string): '), headNodeSize, _);
        //    dataNodeSize = cli.interaction.promptIfNotGiven($('Data node size (string): '), dataNodeSize, _);

        //} else if (utils.ignoreCaseEquals(osType, 'linux')) {
        //    sshUserName = cli.interaction.promptIfNotGiven($('SSH user name: '), sshUserName, _);
        //    sshPassword = cli.interaction.promptIfNotGiven($('SSH password: '), sshPassword, _);
        //}
        ...
        var clusterCreateParameters = {
            ...
            rdpPassword: "passWord!23", //rdpPassword,
            rdpAccessExpiry: Date.now(), //rdpAccessExpiry,
